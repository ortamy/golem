#!/usr/bin/env python3
# tools/checkers/check-tahor.py — проверка чистоты языка (v5.0)
...
def update_metadata_fields(content: str) -> str:
    ...
    counter_match = re.search(r'[-*]\s*\*\*Проверок на чистоту языка:\*\*\s*(\d+)', content)
    if counter_match:
        count = int(counter_match.group(1)) + 1
        content = re.sub(
            r'[-*]\s*\*\*Проверок на чистоту языка:\*\*\s*\d+',
            f'- **Проверок на чистоту языка:** {count}',
            content
        )
...
def fix_tahor(text, replacements, mega_regex):
...
def check_one_file(md_file, replacements, fast_filter, mega_regex, scan_cache, check_only):
    ...
    if found:
        ...
        if not check_only:
            fixed, reps = fix_tahor(content, replacements, mega_regex)
            if reps > 0:
...
def main():
    ...
    print_header("ЗАГРУЗКА СЛОВАРЕЙ ЧИСТОТЫ ЯЗЫКА", "📖")
    replacements, fast_filter, mega_regex = build_replacement_map(rebuild=rebuild_cache)
    
    if not replacements:
        print_error("❌ Словари пусты. Запустите с --rebuild или создайте cache-tahor.json")
        return 1
    
    print(f"✅ Загружено категорий: 20")
    ...
    label = "ПРОВЕРКА ЧИСТОТЫ ЯЗЫКА" if check_only else "ОЧИЩЕНИЕ ЯЗЫКА"
    ...
    if not files_with_issues:
        print_success("🎉 Язык чист. Подмен не найдено.")
        return 0

    print(f"\n📁 Файлов с подменами: {len(files_with_issues)}")
    ...
    if check_only:
        print_hint("\nДля очищения: python tools/checkers/check-tahor.py --fix")
        print_hint("Перестроить кэш:  python tools/checkers/check-tahor.py --rebuild")
    else:
        print_success(f"\n✅ Очищено файлов: {len(files_with_issues)}")
