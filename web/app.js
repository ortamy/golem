...
(function() {
    ...
    function openFileMobile(p) {
        ...
        fetchText(url, function(md) {
            ...
            var html = parseMD(md);
            html = html.replace('<h1>', '<h1>' + iconImg);
            c.innerHTML = html+renderRelatedMobile(p);
            addToHistory(p);
        }, function() { c.innerHTML = '<div style="color:#ff4d00;padding:40px;">Ошибка: '+esc(p)+'</div>'; });
    }
