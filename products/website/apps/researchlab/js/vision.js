...
const Vision = (function() {
  ...
  function setMode(mode) {
    ...
    if (mode === 'huggingface') {
      ...
      hint.textContent = '';
      hint.appendChild(document.createTextNode('Получите бесплатный ключ на '));
  ...
  function renderHistory() {
    if (state.history.length === 0) {
      ...
      empty.textContent = 'Пока нет выполненных анализов.';
    ...
    items.forEach(function(entry) {
      ...
      item.tabIndex = 0;
      item.addEventListener('click', function() { loadHistoryItem(entry.id); });
      item.addEventListener('keydown', function(event) {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault();
          loadHistoryItem(entry.id);
        }
      });
      ...
      image.alt = 'thumb';
      if (typeof entry.imageData === 'string' && /^data:image\/(png|jpeg|jpg|webp);base64,/i.test(entry.imageData)) {
      ...
      preview.textContent = entry.description || '';
      const timestamp = document.createElement('div');
      timestamp.className = 'timestamp';
      timestamp.textContent = dateStr + ' · ' + modeLabel;
