...
const PageController = (function() {
  ...
  function renderDictionaries(container, data) {
    ...
    container.innerHTML = '<div class="research-page-head">' +
      ...
      '<p class="research-description">' + escapeHtml(dictionary.description || '') + '</p>' +
  ...
  function renderManifestPage(container, data) {
    ...
    var cardHtml = cards.map(function(card, index) {
      ...
      return '<article class="manifest-principle" style="animation-delay: ' + (index * 35) + 'ms">' +
        '<div class="manifest-index">' + String(index + 1).padStart(2, '0') + '</div>' +
        '<h3>' + escapeHtml(card.title || 'Раздел манифеста') + '</h3>' +
        '<div class="manifest-principle-content">' + content + '</div>' +
        '</article>';
    }).join('');

    container.innerHTML = '<div class="manifest-page">' +
      '<header class="manifest-hero">' +
      '<div class="manifest-watermark" aria-hidden="true">א ב ג ד</div>' +
      ...
      '</header>' +
      '<section class="manifest-section" aria-labelledby="manifest-cards-title">' +
      '<div class="manifest-section-heading"><div><h2 id="manifest-cards-title">Карта смыслов</h2>' +
      '<p>Опорные положения для чтения, разбора и восстановления утраченных образов.</p></div></div>' +
      '<div class="manifest-principles">' + (cardHtml || '<div class="lab-alert lab-alert-info">Карточки манифеста пока не заполнены.</div>') + '</div>' +
      '</section>' +
      '</div>';
  }
  ...
  function render(moduleId, container, parsed) {
    ...
    if (container.dataset.loaded && container.innerHTML.trim() !== '') return;

    switch (moduleId) {

      // ===== СТАТИЧЕСКИЕ МОДУЛИ (HTML встроен) =====
      case 'dashboard':
        container.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">' +
          '<h1 style="margin-bottom:0"><img src="../../assets/icons/32/archaeology/testtube.png" width="32" height="32" alt="Лаборатория" style="vertical-align: middle; margin-right: 6px;"> Рабочий стол исследователя</h1>' +
          '<button type="button" class="lab-btn lab-btn-secondary lab-btn-sm" onclick="Dashboard.reload()">⟳ Обновить статистику</button>' +
          '</div>' +
          '<p class="subtitle">Статистика проекта и инструменты для анализа артефактов, символов, текстов, улик и скрытых подмен.</p>' +
          '<div id="dashboard-widgets" class="dashboard-widgets"><div class="lab-spinner show"><div class="loader"></div><div class="spinner-text">Загрузка статистики…</div></div></div>';
      ...
      case 'scripture-reader':
        container.innerHTML = '<div class="research-page-head scripture-reader-head">' +
          ...
          '<nav class="scripture-navigation scripture-navigation-top" id="scripture-navigation-top" style="display:none;" aria-label="Навигация по стихам сверху">' +
          '<button type="button" class="lab-btn lab-btn-secondary" id="scripture-prev-top">← Предыдущий стих</button>' +
          '<button type="button" class="lab-btn lab-btn-secondary" id="scripture-next-top">Следующий стих →</button>' +
          '</nav>' +
          ...
          '</section>' +
          '<aside id="scripture-loss-drawer" class="scripture-loss-drawer" aria-hidden="true" aria-labelledby="scripture-loss-title">' +
