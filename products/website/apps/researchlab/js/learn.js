...
(function(root) {
  ...
  var letters = [];
  var state = { view:'home', lesson:null, game:null, timer:null, course:null, trainer:null };
  ...
  function renderHome() {
    ...
    return '<div class="learn-state-panel"><div><h2>Состояние Свивы</h2><p>Результаты уроков и рекорд сохраняются только в этом браузере.</p></div><button type="button" class="lab-btn lab-btn-secondary learn-danger" onclick="LearnLab.reset()">Сбросить прогресс</button></div>' +
      ...
      '<div class="learn-mode-grid"><button class="learn-mode-card" type="button" onclick="LearnLab.openLessons()"><span class="learn-mode-icon" aria-hidden="true">𐤀</span><h2>Изучение иврита</h2><p>22 урока по буквам: название, образ, значение и обратное узнавание символа.</p><div class="learn-mode-meta"><span>' + completed + '/22 уроков</span><span>→</span></div><div class="learn-progress-bar" aria-label="Прогресс ' + completed + ' из 22"><span style="width:' + (completed / 22 * 100) + '%"></span></div></button><button class="learn-mode-card" type="button" onclick="LearnLab.openGame()"><span class="learn-mode-icon" aria-hidden="true">𐤔</span><h2>Угадай образ</h2><p>Игровой раунд на скорость: увидьте знак, выберите предметный образ и соберите серию.</p><div class="learn-mode-meta"><span>Рекорд: ' + record() + ' очков</span><span>→</span></div></button><button class="learn-mode-card" type="button" onclick="LearnLab.openCourses()"><span class="learn-mode-icon" aria-hidden="true">𐤅</span><h2>Курсы</h2><p>Практические курсы: палео-механика, без воды, результат после каждого модуля.</p><div class="learn-mode-meta"><span>' + coursesCount() + ' курсов</span><span>→</span></div></button><button class="learn-mode-card" type="button" onclick="LearnLab.openTrainer()"><span class="learn-mode-icon" aria-hidden="true">𐤏</span><h2>Палео-тренажёр</h2><p>Крупные палео-буквы: увидь образ, назови функцию, собери смысл. Пиши свой ответ или генерируй новое слово.</p><div class="learn-mode-meta"><span>Начать</span><span>→</span></div></button></div>';
  }
  ...
  var api = {
    ...
    reset: function() { if (!window.confirm('Сбросить весь прогресс обучения, рекорд игры и прогресс тренажёра?')) return; localStorage.removeItem(PROGRESS_KEY); localStorage.removeItem(RECORD_KEY); state.trainer = null; navigate([]); }
  };
