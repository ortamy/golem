...
const BoardLib = (function() {
  ...
  function view(id) {
    ...
    if (window.LabModal) {
      ...
      var footer = '<button class="lab-btn lab-btn-primary lab-btn-sm" onclick="LabExport.copyText(' + JSON.stringify(content) + ')">📋 Копировать</button>' +
      ...
      LabModal.show('🗂 ' + (board.title || 'Просмотр доски'), body, footer);
    } else {
      // fallback
      alert(content);
    }
