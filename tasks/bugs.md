...
- [x] P0 · хедер / hero / карточки · битые изображения в изолированном smoke-сервере: ссылки `../../assets/...` выходили за корень изоляции (на сайте ассеты на месте — `products/website/assets/`); сделано локальное зеркало `apps/researchlab/assets/` (86 файлов: иконки 32px + `alephy-logo-2.png`), ноль правок ссылок, изолированные кадры теперь пиксель-в-пиксель как сайт · проверено HEAD-запросами (200 по всем ранее 404-URL)
