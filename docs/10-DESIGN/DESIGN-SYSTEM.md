...
- **Палитры тем монолитны** (блок «МОНОЛИТНЫЕ ПАЛИТРЫ ТЕМ» в `css/tokens.css` и пресеты в `user-preferences.js`): `light/parchment` = бежевая, `white` = белая, `dark` = чёрная (тёплый чёрный), новая `brown` = коричневая. Внутри темы нет смешения пастельно-бежевого/серого с коричневым; семантика (status/warn/error/info) — тёплые приглушённые тона той же семьи.
- **Тема `brown`** добавлена в переключатель `LabTheme.themes` (`light → parchment → white → brown → dark`) и в панель настроек (`PRESETS.theme`).
