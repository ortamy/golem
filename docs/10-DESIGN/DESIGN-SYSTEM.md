...
| `.glass-search-panel` | полноширинная панель под sticky-шапкой, два состояния: закрытое `height: 0` + `opacity: 0` + `overflow: hidden`; открытое около `64px` + `opacity: 1`; открытое состояние переиспользует tint/blur/золотую нижнюю кромку `.glass-topbar.is-scrolled`; transition 200ms ease; ≤767px solid без blur, reduced-motion без transition | глобальный поиск Research Lab `#globalSearch`, открывается `.search-toggle`, содержит `#gs-input` и существующие результаты `#gs-results` |
...
## Этимологическая модалка (Research Lab, фаза 1)

Компонент открывается кликом или клавишами Enter/Space по карточке корня в `#root-dictionary`. Он использует общий `#labModal.modal-window.glass-modal`, поэтому сохраняет Escape, клик по backdrop и кнопку закрытия, а на ширине до 767px занимает весь viewport без горизонтального overflow.

- **Шапка:** исходный глиф, транслитерация и значение корня.
- **Пра-форма:** поле `proto`, крупная serif-типографика с ведущей `*`.
- **Когнаты:** список чипов `lang + form + meaning`.
- **Этапы:** вертикальный timeline `period → form → note` с золотыми узлами.
- **Семантические сдвиги:** строки `from → to` с типом сдвига.
- **Уверенность:** `.rem-confidence-fact` (эмет-золото), `.rem-confidence-interpretation` (нейтральный), `.rem-confidence-hypothesis` (приглушённый).
- **Источники и заметки:** мелкий вторичный текст внизу.
- **Состояния:** skeleton во время lazy-fetch, «Разбор готовится» для HTTP 404/отсутствующего файла, retry для прочих ошибок загрузки. Все динамические значения экранируются; `prefers-reduced-motion` отключает pulse skeleton и сохраняет базовую модальную анимацию без движения.

Контракт lazy-файла `data/roots/etymology/{id}.json`: `proto: string`, `cognates: [{ lang, form, meaning }]`, `stages: [{ period, form, note }]`, `shifts: [{ from, to, type }]`, `confidence: fact | interpretation | hypothesis`, `sources: string[]`, `notes: string`.
